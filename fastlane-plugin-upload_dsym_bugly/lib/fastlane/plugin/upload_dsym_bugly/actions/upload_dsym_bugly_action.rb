...
require_relative '../helper/upload_dsym_bugly_helper'

module Fastlane
  module Actions
    module SharedValues
      UPLOAD_DSYM_BUGLY_CUSTOM_VALUE = :UPLOAD_dsym_BUGLY_CUSTOM_VALUE
    end

    class UploaddsymBuglyAction < Action
      def self.run(params)
      require 'json'
...
cmd = "curl -k \"https://api.bugly.qq.com/openapi/file/upload/symbol?app_key=#{params[:app_key]}&app_id=#{params[:app_id]}\"" +  "--form \"api_version=#{params[:api_version]}\"" + " --form \"app_id=#{params[:app_id]}\"" + " --form \"app_key=#{params[:app_key]}\""  + " --form \"symbolType=#{params[:symbol_type]}\"" + " --form \"bundleId=#{params[:bundle_id]}\"" + " --form \"productVersion=#{params[:product_version]}\"" + " --form \"fileName=#{params[:file_name]}\"" + " --form \"file=@#{params[:file_path]}\"" + " -o " + json_file
sh(cmd)
...
`rm upload_dsym_to_bugly_result.json`
      ...
      def self.output
        ...
        # Example
        [
          ['UPLOAD_DSYM_BUGLY_CUSTOM_VALUE', 'A description of what this value contains']
        ]
      end
      ...
      def self.authors
        ...
        ["https://github.com/JKangTan/upload_dsym_bugly.git"]
      end
